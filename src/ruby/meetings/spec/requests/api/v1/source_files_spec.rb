...
RSpec.describe "Api::V1::SourceFiles", type: :request do
  describe "GET /api/v1/source_files" do
    let(:source_file){create(:source_file)}
    let(:headers){{'HTTP_ACCEPT' => "application/vnd.api+json"}}
    it "it returns a 200" do
      get api_v1_source_file_path(source_file),headers: headers
      expect(response).to have_http_status(200)
    end
    describe "POST /api/v1/source_files" do
      let(:json_api_data) do
        {
          "data" =>  {
            "type" =>  "source-files",
            "attributes" =>  {
              "location" =>  "Ember Hamster2",
              "uri" =>  "an entry" 
            }
          }
        }.to_json
      ...
      it 'returns a 201' do
        post api_v1_source_files_path,params: json_api_data,headers: headers
        expect(response).to have_http_status(201)
      end

      it 'returns a json-api response' do
        post api_v1_source_files_path,params: json_api_data, headers: headers
        expect(response.body).to be_jsonapi_response_for('source-files')
      end
    end
